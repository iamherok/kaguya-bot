...
module.exports = handle = (client, Client) => {
    try {
        ...
        Client.cmd.on('ig', async (data) => {
            if(isLimit(data.sender)) return data.reply(mess.limit)
            if(data.body == "") return data.reply(`Send orders  *${data.prefix}ig [ link ]*\nEx : ${data.prefix}ig https://www.instagram.com/p/CJ8XKFmJ4al/?igshid=1acpcqo44kgkn`)
            data.reply(mess.wait)
        ...
        Client.cmd.on('revoke', (data) => {
            ...
            data.reply(`*The linkgroup has been successfully reset by admin* @${data.sender.split('@')[0]}`)
        })
        Client.cmd.on('group', (data) => {
            ...
            if(data.args[0] && data.args[0].toLowerCase() == 'open') {
                ...
                data.reply(`*Group has been opened by admin* @${data.sender.split('@')[0]}`)
            } else if(data.args[0] && data.args[0].toLowerCase() == 'close') {
                client.groupSettingChange(data.from, GroupSettingChange.messageSend, true)
                data.reply(`*Group has been closed by admin* @${data.sender.split('@')[0]}`)
            } else {
				let po = client.prepareMessageFromContent(data.from, {
					"listMessage":{
                  "title": "*[Kaguya🌸]*",
...
`
        ...
        Client.cmd.on('*', async (data) => {
            ...
            switch(command.toLowerCase()) {
                ...
                case 'hi':
                case 'kaguya':
 	                  const mediaMsg = await client.prepareMessageMedia(fs.readFileSync('./lib/fake.jpg'), 'imageMessage')
                     const buttonMessage = {
...
`
                ...
                case 'jadwalshalat':
                    if(isLimit(data.sender)) return data.reply(mess.limit)
                    if(data.body == "") return data.reply(`Send orders  *${data.prefix}${data.command} [ teks ]*\nEx : ${data.prefix}${data.command} jakarta`)
                    data.reply(mess.wait)
                ...
                case 'jadwaltv':
                    if(isLimit(data.sender)) return data.reply(mess.limit)
                    if(data.body == "") return data.reply(`Send orders  *${data.prefix}${data.command} [ teks ]*\nEx : ${data.prefix}${data.command} antv`)
                    data.reply(mess.wait)
